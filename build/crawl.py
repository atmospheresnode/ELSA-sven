...
def mission_list():
    ...
    list_of_missions = []
    lines = urllib.request.urlopen(starbase_url).readlines()
...
#            print starbase_url_product
            ...
            starbase_label_product =  etree.ElementTree(file=urllib.request.urlopen(starbase_url_product))
...
def facility_list():
    ...
    for lina in lines:
        ...
        if '.xml' in lina:  
            ...
            tree = etree.ElementTree(file=urllib.request.urlopen(new_starbase_url))
            root = tree.getroot()
...
def facility_tuple():
    ...
    tuple_of_facilities = list(zip(list_of_facilities_for_url, list_of_facilities_title))
...
#    print investigation_url
    investigation = etree.ElementTree(file=urllib.request.urlopen(investigation_url))
...
def instrument_host(context_dict):
    ...
    for instrument_host in instrument_host_string_list:
        ...
        instrument_host_tree = etree.ElementTree(file=urllib.request.urlopen(instrument_host_url))
        ...
        for element in instrument_host_tree.findall(lidvid_reference):
            if element.text[21:31] == 'instrument':
                ...
                instrument_tree = etree.ElementTree(file=urllib.request.urlopen(instrument_url))
                ...
                target_tree = etree.ElementTree(file=urllib.request.urlopen(target_url))
