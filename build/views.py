...
def bundle(request, pk_bundle):
    ...
    if request.user == bundle.user:
        ...
        if request.method == 'POST':
            print("before checking if form_netcdf if valid")
            if form_netcdf.is_valid():
                ...
                netcdf_collection = AdditionalCollections.objects.filter(collection_name=collection).first()

                print(netcdf_collection)
                print(netcdf_collection.directory())
...
def variable_coord_to_product(bundle, netcdf_objs, collection_directory):
    ...
    for nc_obj in netcdf_objs:
        try:
            ...
            os.rename(nc_path, os.path.join(collection_directory, os.path.basename(nc_obj.file.path)))
            nc_path = os.path.join(collection_directory, os.path.basename(nc_obj.file.path))
            print(nc_path)
