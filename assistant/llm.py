...
class GeminiClient:
    ...
    def open_stream(self, system_prompt, contents, tools=None):
        """Try each model in the chain; return (model_name, streaming_response).

        A 429 means that model's own free-tier bucket is exhausted; 5xx or a
        network error means it's overloaded — either way the next model may
        still work. Raises QuotaExhausted / LLMUnavailable when the chain ends.
        """
        body = {
